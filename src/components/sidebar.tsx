    ...
    <TooltipTrigger className="w-full">
      <BjListItem url={item.broad_info[0].url}>
        ...
        <div className="grow overflow-hidden">
          <h3 className="truncate text-xl font-semibold leading-none">
            {item.user_nick}
          </h3>
...
function Sidebar() {
  ...
  if (isError) return <p>로그인 정보를 확인해주세요.</p>;
  if (!data || data.data.length === 0) return <p>즐겨찾기한 BJ가 없습니다.</p>;

  return (
    <aside
      aria-label="BJ 목록"
      className="scrollbar-thin scrollbar-rounded-full reset fixed h-[calc(100%-var(--gnb-height))] w-[inherit] overflow-auto px-2 pb-4 text-foreground"
    >
